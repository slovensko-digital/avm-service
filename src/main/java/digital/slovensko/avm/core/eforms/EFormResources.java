...
public class EFormResources {
    ...
    public static EFormAttributes tryToLoadEFormAttributesFromEmbeddedXdc(DSSDocument document) {
        try {
            var xdc = EFormUtils.getXmlFromDocument(document).getDocumentElement();
            var formUri = EFormUtils.getFormUri(xdc);
            if (formUri == null)
                return null;

            var schemaNode = (Element) EFormUtils.getNoTextFirstChild(EFormUtils.getElementFromXdc(xdc, "UsedXSDEmbedded"));
            var schema = EFormUtils.transformElementToString(schemaNode);
            var transformationNode = (Element) EFormUtils.getNoTextFirstChild(EFormUtils.getElementFromXdc(xdc, "UsedPresentationSchemaEmbedded"));
            var transformation = EFormUtils.transformElementToString(transformationNode);
    ...
    private ArrayList<ManifestXsltEntry> getManifestXsltEntries(NodeList nodes) {
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (mediaDestinationTypeDescription == null) {
                ...
                var xsltString = EFormUtils.getResource(SOURCE_URL + url + "/" + fullPath);
                if (xsltString == null)
                    continue;
    ...
    private boolean findResources() throws XMLValidationException {
        ...
        var manifest_xml = EFormUtils.getResource(SOURCE_URL + url + "/META-INF/manifest.xml");
        ...
        var parsed_manifest_xml = EFormUtils.getXmlFromDocument(new InMemoryDocument(manifest_xml, "manifest.xml"));
        ...
        var xsltString = EFormUtils.getResource(SOURCE_URL + url + "/" + entry.fullPath());
        if (xsltString == null)
            return false;

        var xsltDigest = EFormUtils.computeDigest(xsltString, canonicalizationMethod, DigestAlgorithm.SHA256, ENCODING);
        ...
        var xsdString = EFormUtils.getResource(SOURCE_URL + url + "/schema.xsd");
        if (xsdString == null)
            return false;

        var xsdDigest = EFormUtils.computeDigest(xsdString, canonicalizationMethod, DigestAlgorithm.SHA256, ENCODING);
    ...
    private boolean findResourcesEmbedded() throws XMLValidationException {
        if (schema == null) {
            var schema_raw = EFormUtils.getResource(url);
            if (schema_raw == null)
        ...
        if (transformation == null) {
            ...
            var transformation_raw = EFormUtils.getResource(transformationUrl);
    ...
    private static EFormResources buildEFormResourcesFromXDC(DSSDocument document, String canonicalizationMethod)
        ...
        var xsdDigest = EFormUtils.getDigestValueFromElement(xdc, "UsedXSDReference");
        var xsltDigest = EFormUtils.getDigestValueFromElement(xdc, "UsedPresentationSchemaReference");

        var xsdIdentifier = EFormUtils.getValueFromElement(xdc, "UsedXSDReference");

        var params = EFormUtils.getXsltParamsFromXsltReference(xdc);
    ...
    private static EFormResources buildEFormResourcesFromEformXml(DSSDocument document, String canonicalizationMethod,
            String xsdIdentifier, XsltParams xsltParams)
            throws XMLValidationException {
        var xml = EFormUtils.getXmlFromDocument(document).getDocumentElement();
        var formUri = EFormUtils.getNamespaceFromEformXml(xml);
